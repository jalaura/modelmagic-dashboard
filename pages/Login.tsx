    ...
    <div className="min-h-screen bg-[#F5F3F0] flex items-center justify-center p-4 font-sans">
      <div className="bg-white max-w-md w-full p-8 rounded-xl border border-[#E5E7EB] shadow-sm">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          {status === 'error' && (
             <p className="text-sm text-[#EF4444] font-medium">User not found. Try sarah@luxeboutique.com or admin@modelmagic.com</p>
          )}

          <button 
            type="submit" 
            disabled={status === 'sending'}
            className="w-full bg-[#1A1A1A] text-white py-3 rounded-md font-bold hover:bg-black transition-colors flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {status === 'sending' ? (
              <>
        ...
        <div className="mt-8 pt-6 border-t border-[#E5E7EB] text-center">
            ...
            <div className="space-y-2 text-sm text-[#6B7280]">
                <div onClick={() => setEmail('sarah@luxeboutique.com')} className="cursor-pointer hover:text-[#1F6B55]">Client: sarah@luxeboutique.com</div>
